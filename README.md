...
1. Load/edit a photo, open **Review**, select **ChatGPT — Manual import**, and optionally describe your aesthetic intent. **Allow detail adjustments (clarity only)** defaults on; uncheck before exporting if you want detail to remain manual.
...
Every export clears the previous proposal. Changing source, dimensions, sliders, curves, masks, intent, detail permission, or provider invalidates the export; refreshing the page requires re-export. Switching providers cancels pending work but does not undo edits already applied. Import and smart-quote repair validate against the exported detail permission and clarity baseline, not permission granted afterward. Use only ChatGPT's response for the matching files: ABEL can enforce an unchanged export baseline but cannot attest which image an external conversation reviewed.
...
The **Review** tab is available on desktop and in the mobile bottom navigation. Review is subjective advice, not an objective image-quality measurement. Global slider values are **absolute targets**, not deltas. Each intensity's Adaptive has its own global base plus up to three soft regional masks. Crop is advice only; no crop, retouching, objects or curves are changed. Detail edits are forbidden except explicitly enabled, bounded clarity (below). Pending slider edits are retained before the transaction. **Undo review** or toolbar undo restores prior global settings and mask pixels/count; toolbar redo restores the latest selected recipe. Changed source, dimensions, sliders, curves, masks, intent, detail permission, action, intensity, strength or connection invalidate pending work. Cancel, revoked consent, provider switching, stale responses, errors and invalid JSON never trigger automatic application. Empty/zero-strength choices never substitute another recipe. There is no provider fallback or automatic request on reload/provider switch.

### Optional clarity, not automatic detail repair

**Allow detail adjustments (clarity only)** defaults checked for Azure, Gemini, local Qwen and manual export. It permits—but never forces—bounded clarity suggestions. The current checked/unchecked choice survives photo changes in this tab, but is not saved with connection preferences. Uncheck **before** requesting/exporting a review to keep detail manual; intensity and strength never enable it. Changing permission cancels pending work and clears cached proposals without undoing existing edits. Upload consent remains unchecked on startup and each new photo.

Clarity uses the existing renderer's simplified source-neighbor local contrast operation, globally and in soft masks. There is **no texture control or texture rendering operation**, so texture is not proposed or aliased to clarity. Sharpening, noise reduction, dehaze, grain, vignette, geometry and curves are not AI-controlled. The clarity calculation compares source neighbors to the source center, not the post-exposure color, preventing tonal edits from generating false detail contrast in flat areas.

Both server and browser enforce maximum absolute clarity changes of **5 / 10 / 15** for Refine / Balanced / Expressive, relative to the frozen global baseline and within the renderer's −100…100 range. Regional clarity offsets start at zero, and the **sum of absolute offsets** across new regions has the same per-intensity limit. Existing masks and manual detail settings remain untouched.

After Apply, open **⋯ → Details** beside the intensity buttons. When the selected recipe proposes clarity, toggle it off to keep the **same lighting/color treatment with AI clarity omitted**, not an independently generated alternative. Otherwise the disabled control says **No AI clarity suggested**; older responses without clarity require a new review to suggest it. The Review adjustment list shows global and regional clarity values when included. Original global clarity and existing masked clarity remain intact; clarity-only new regions are omitted. Toggle, intensity, mode and strength changes reuse the same response and baseline, never call AI again, and share one undoable transaction. At 0% strength the complete baseline is preserved. PNG/JPEG/WebP export uses the same masked renderer.

The model sees only the existing reduced JPEG (cloud/manual up to 1280 pixels, local up to 768), not a full-resolution detail crop. The prompt requires visible evidence, omission under uncertainty, protection of intentional blur/haze/skin, and avoidance of halos/noise amplification. It cannot recover missing detail, and may validly propose no clarity change. Inspect the result rather than treating permission as a recommendation.

`node scripts/check-detail-review.cjs` (with an existing Playwright installation, or `PLAYWRIGHT_MODULE` pointing to it) checks real synthetic pixel differences, same-treatment detail toggles, original masked clarity, zero strength, undo/redo, PNG export parity, Azure/checkbox defaults, new-photo consent reset and non-overlapping 320px mobile controls. All inference is intercepted; it spends no provider allowance. `BASE_URL` optionally tests deployed public assets.
...
- `validateRequest(value)` / `validateReview(value, request)`: return independent validated data or throw `Error`. Missing permission forbids clarity; opt-in requires a finite current clarity baseline. No silent filtering, clamps or numeric coercion in validation. Slider values must be finite and within range; step sizes guide recommendations/UI rather than rejecting interpolated current values.
- `reviewSchema`: default detail-off closed JSON schema. `schemaForRequest(request)` conditionally includes clarity in global/regional key enums. Each provider derives its compatible grammar from that request-specific schema. `server/schema.js` derives the Gemini-compatible version: required fields, closed objects, key enums, and numeric bounds remain; string/array size limits become descriptions to avoid provider schema-complexity rejection. Both runtime validators still enforce every size limit, key-specific range, baseline-relative detail bound, and duplicate check.
...
All three fields are required; empty intent is allowed. All 34 current adjustments are required. For backwards compatibility, omitting `allowDetails` means **false**; new clients explicitly send that boolean. Only when `allowDetails: true`, also send `detailAdjustments: { clarity: currentState.clarity }` (required, exact fields, finite −100…100). Supplying detail settings while permission is false/omitted is rejected. `adjustments` stays the same 34-field map. There are no URL, model, crop, tool, or arbitrary settings fields. Response shape is unchanged; unsolicited clarity or out-of-policy values reject the entire response.
