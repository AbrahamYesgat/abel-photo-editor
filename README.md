...
The **Review** tab is available on desktop and in the mobile bottom navigation. Review is subjective advice, not an objective image-quality measurement. Global slider values are **absolute targets**, not deltas. Adaptive has its own global base plus up to three soft regional masks. Crop is advice only; no crop, retouching, objects, curves or detail controls are changed. Pending slider edits are retained before the transaction. **Undo review** or toolbar undo restores prior global settings and mask pixels/count; toolbar redo reapplies both. Changed source, dimensions, sliders, curves, masks, intent, action, strength or connection invalidate pending Gemini work. Cancel, revoked consent, provider switching, stale responses, errors and invalid JSON never trigger automatic application. No-change or zero-strength recipes report that nothing was applied, without substituting the other alternative. There is no provider fallback or automatic request on reload/provider switch.
...
Strength is chosen **before the action**: 0% changes nothing and creates no masks; 50% interpolates global targets from the review baseline and halves regional offsets; 100% uses the full proposal. It does not shrink or harden masks. Applied recipes cannot be reapplied. For a different choice, Undo and request a fresh review; changing Gemini's next action does not undo the already-applied edit. Hold the photo or **Hold to see before review** to compare against the complete pre-review composite (including old masks); release to return. Comparison also works after redo unless another edit invalidates it. PNG/JPEG/WebP and scaled export include the mask composite and never export the held comparison. Library sidecars/IndexedDB retain region names, reasons, blending and bitmap masks. Cropping still clears masks because dimensions change; undoing the crop restores its prior masks. Crop redo remains unsupported.
...
Empty alternatives mean “no major lighting or color edit needed”; empty improvements displays “No major issue identified.” Unknown fields, missing fields, duplicate JSON members/adjustments/categories/list entries, invalid numbers, degenerate/hard geometry, unsupported controls, excessive masks, malformed output, refusal, and truncation are rejected by the server and again in the browser, including before application. Providers share the base prompt/schema; Gemini additionally receives the bounded detailed audit. Only an explicit Gemini **Review & apply** click authorizes application after validation. Manual ChatGPT imports and local Qwen reviews remain review-first. Refresh an already-open editor after updates; older responses missing `adaptive` are deliberately rejected, not silently converted.
