...
1. Load/edit a photo, open **Review**, select **ChatGPT — Manual import**, and optionally describe your aesthetic intent. If wanted, enable **Allow detail adjustments (clarity only)** before exporting; it is off by default.
...
**Allow detail adjustments (clarity only)** is unchecked on startup and whenever a new photo loads. It is not saved with connection preferences; intensity and strength never enable it. Enable it **before** requesting/exporting a review. Changing permission cancels pending work and clears cached proposals without undoing existing edits.
...
After Apply, **Details** appears beside the photo only when the selected recipe proposes clarity. Toggle it off to keep the **same lighting/color treatment with AI clarity omitted**, not an independently generated alternative. Original global clarity and existing masked clarity remain intact; clarity-only new regions are omitted. Toggle, intensity, mode and strength changes reuse the same response and baseline, never call AI again, and share one undoable transaction. At 0% strength the complete baseline is preserved. PNG/JPEG/WebP export uses the same masked renderer.
...
`node scripts/check-detail-review.cjs` (with an existing Playwright installation, or `PLAYWRIGHT_MODULE` pointing to it) checks real synthetic pixel differences, same-treatment detail toggles, original masked clarity, zero strength, undo/redo, PNG export parity, new-photo permission reset and non-overlapping 320px mobile controls. All inference is intercepted; it spends no provider allowance. `BASE_URL` optionally tests deployed public assets.
